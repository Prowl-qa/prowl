...
### Added
- **Unified native selector engine (PROWL-060).** The native selector dialect —
  the grammar for `id=`/`label=`/`text=`/`role=` (and `:focus`), the per-platform
  attribute mapping tables, the ranking order, and the host-side matching
  semantics — now lives in one module, `src/selector/native.ts`, instead of being
  duplicated across each mobile driver's locator translation and each analyzer's
  selector ranking. Both mobile drivers parse through the shared
  ...
  `rankNativeSelectors`, so the dialect can never drift between platforms. The
  ...
  (macOS matching is left untouched; a `MACOS_MATCH_DIALECT` extension point is in
  place for the deferred migration).
